...
const hooksDir = 'c:/Users/Dhanush/OneDrive/Desktop/PROJECTS/EventDecor/frontend/src/admin/hooks';
const compDir =
  'c:/Users/Dhanush/OneDrive/Desktop/PROJECTS/EventDecor/frontend/src/admin/components';

// 1. Fix logger imports
let submitContent = fs.readFileSync(path.join(hooksDir, 'useProductSubmission.js'), 'utf8');
submitContent = submitContent.replace(
  "import logger from '../../../utils/logger';",
  "import logger from '../../utils/logger';",
);
fs.writeFileSync(path.join(hooksDir, 'useProductSubmission.js'), submitContent);

let aiContent = fs.readFileSync(path.join(hooksDir, 'useProductAI.js'), 'utf8');
aiContent = aiContent.replace(
  "import logger from '../../../utils/logger';",
  "import logger from '../../utils/logger';",
);
fs.writeFileSync(path.join(hooksDir, 'useProductAI.js'), aiContent);

// 2. Fix LivePreviewCard
let previewContent = fs.readFileSync(path.join(compDir, 'LivePreviewCard.jsx'), 'utf8');
// It duplicated lines starting from "{/* Live Catalog Preview Card */}"
// Let's just slice it perfectly.
// Since the top half is correct and the bottom half is duplicated, we can just slice it manually
let lines = previewContent.split('\n');
let correctLines = lines.slice(0, 139); // Up to the span
// The span was closed at 139?
// Wait, 139 is "        {/* Live Catalog Preview Card */}" in the duplicated part.
// So 0 to 138 is the correct content except it needs closing tags.

const correctCard = `import React from 'react';
...
`;
fs.writeFileSync(path.join(compDir, 'LivePreviewCard.jsx'), correctCard);

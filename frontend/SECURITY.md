...
- **Connect/API (`connect-src`)**: 'self', `siri-arts-n-crafts.onrender.com` (REST and WSS), `api.razorpay.com`
- **Iframes (`frame-src`)**: 'self', `checkout.razorpay.com`

*Note: If integrating a new third-party service (e.g., a chat widget or analytics tool), the CSP headers in `vercel.json` and `nginx.example.conf` MUST be updated to whitelist the specific domains.*

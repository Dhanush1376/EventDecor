import React from 'react';
import { SkeletonBox, SkeletonTextLine, SkeletonBadge } from '../../ui/Skeletons';

export function AdminRentalDrawerSkeleton() {
  return (
    <div className="p-5 space-y-6 text-left admin-section-root">
      {/* Drawer Header */}
      <div className="flex items-center justify-between pb-4 border-b border-[var(--admin-border-subtle)]">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <SkeletonTextLine width="120px" height="18px" />
            <SkeletonBadge width="80px" height="20px" />
          </div>
          <SkeletonTextLine width="150px" height="12px" />
        </div>
        <SkeletonBox width="28px" height="28px" rounded="sm" />
      </div>

      {/* Rental Period & Security Deposit Strip */}
      <div className="grid grid-cols-2 gap-3 p-3.5 rounded bg-[var(--admin-surface-muted)] border border-[var(--admin-border-subtle)]">
        <div className="space-y-1">
          <SkeletonTextLine width="75px" height="10px" />
          <SkeletonTextLine width="110px" height="13px" />
        </div>
        <div className="space-y-1">
          <SkeletonTextLine width="85px" height="10px" />
          <SkeletonTextLine width="90px" height="13px" />
        </div>
      </div>

      {/* Rented Product Preview */}
      <div className="p-3.5 rounded border border-[var(--admin-border-subtle)] bg-[var(--admin-surface)] space-y-3">
        <SkeletonTextLine width="110px" height="13px" />
        <div className="flex items-center gap-3">
          <SkeletonBox width="52px" height="52px" rounded="sm" className="shrink-0" />
          <div className="space-y-1.5 flex-1 min-w-0">
            <SkeletonTextLine width="160px" height="14px" />
            <div className="flex justify-between">
              <SkeletonTextLine width="70px" height="11px" />
              <SkeletonTextLine width="60px" height="13px" />
            </div>
          </div>
        </div>
      </div>

      {/* Customer & ID Proof Check */}
      <div className="space-y-3">
        <SkeletonTextLine width="120px" height="13px" />
        <div className="p-3 rounded bg-[var(--admin-surface-muted)] space-y-2">
          <div className="flex items-center gap-2.5">
            <SkeletonBox width="32px" height="32px" rounded="full" />
            <SkeletonTextLine width="130px" height="13px" />
          </div>
          <div className="flex items-center gap-2 pt-1">
            <SkeletonBadge width="65px" height="18px" />
            <SkeletonTextLine width="110px" height="11px" />
          </div>
        </div>
      </div>

      {/* Action Buttons */}
      <div className="flex items-center gap-2 pt-2">
        <SkeletonBox width="100%" height="38px" rounded="sm" />
        <SkeletonBox width="100%" height="38px" rounded="sm" />
      </div>
    </div>
  );
}
export default AdminRentalDrawerSkeleton;

...
// ═══════════════════════════════════════════════════════════════
// ADMIN SKELETON — layout-preserving skeleton loading
// ═══════════════════════════════════════════════════════════════
      ...
      <div className="p-4 border-b border-[var(--admin-border-subtle)] bg-[var(--admin-bg-subtle)]">
        <div className="flex gap-4">
          {Array.from({ length: cols }).map((_, i) => (
            <AdminSkeleton key={i} className="h-3 rounded flex-1" />
          ))}
        </div>
      </div>
      {Array.from({ length: rows }).map((_, r) => (
        <div key={r} className="p-4 border-b border-[var(--admin-border-subtle)] last:border-b-0">
          <div className="flex gap-4 items-center">
            {Array.from({ length: cols }).map((_, i) => (
              <AdminSkeleton key={i} className={`h-3 rounded flex-1 ${i === 0 ? "max-w-[80px]" : ""}`} />
            ))}
          </div>
        </div>
      ))}
    ...
    <div className="space-y-6 admin-animate-in">
      <div className="flex items-center justify-between">
        <div className="space-y-2">
          ...
          <AdminSkeleton className="w-72 h-4 rounded" />
        </div>
        <AdminSkeleton className="w-32 h-9 rounded-[var(--admin-radius-lg)]" />
      </div>

  ...
  const handleUpload = async (e) => {
    ...
    for (const file of files) {
      ...
      if (!allowedExts.includes(fileExt) || !file.type.startsWith('video/')) {
        toast.error(
          `Invalid video format: ${file.name}. Only MP4, WEBM, MOV, and OGG are permitted.`,
        );
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message ||
          'Failed to upload video. Please verify file signature and try again.',
        { id: toastId },
      );
    } finally {
  ...
  return (
    <div className="flex items-center justify-between gap-3 p-2 bg-neutral-50 rounded-lg border border-neutral-200/50">
      <div className="flex-1 space-y-0.5">
        {label && (
          <span className="text-[11px] sm:text-[11px] sm:text-[11px] font-bold text-neutral-400 uppercase tracking-widest block">
            {label}
          </span>
        )}
        <button

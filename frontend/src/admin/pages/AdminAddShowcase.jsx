...
export function AdminAddShowcase() {
        ...
        try {
          ...
          if (res.success && res.data) {
            ...
            setFormData({
              ...
              image: sc.image || "",
              galleryImages: sc.gallery && sc.gallery.length > 0 ? [...sc.gallery, "", ""].slice(0, 2) : ["", ""],
  ...
  const getStepErrors = () => {
    ...
    if (currentStep === 2) {
      if (formData.rentalPrice === "" || Number(formData.rentalPrice) <= 0) errors.rentalPrice = "A valid rental price is required";
    }
    if (currentStep === 3) {
      if (!formData.description.trim()) errors.description = "Atmospheric narrative description is required";
    }
    return errors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        suggestedProps: formData.suggestedProps ? formData.suggestedProps.split(",").map((p) => p.trim()).filter(Boolean) : [],
  ...
  const colors = formData.colorPalette
    ? formData.colorPalette.split(",").map((c) => c.trim()).filter((c) => c.startsWith("#") || c.startsWith("rgb") || c.length > 2)
    : [];
  ...
  return (
    <div className="max-w-[1280px] mx-auto space-y-6 pb-20 sm:pb-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2 text-[11px] text-[var(--admin-text-secondary)] font-semibold bg-[var(--admin-surface)] border border-[var(--admin-border)] px-3 py-1.5 rounded-full uppercase tracking-wider">
            <span className="px-1.5 py-0.5 bg-[var(--admin-bg-subtle)] border border-[var(--admin-border)] rounded text-[11px] sm:text-[11px]">Alt + →</span>
            <span>Next</span>
            <span className="text-[#E5E7EB]">|</span>
            <span className="px-1.5 py-0.5 bg-[var(--admin-bg-subtle)] border border-[var(--admin-border)] rounded text-[11px] sm:text-[11px]">Ctrl+S</span>
            <span>Save</span>
            <span className="text-[#E5E7EB]">|</span>
            <span className="px-1.5 py-0.5 bg-[var(--admin-bg-subtle)] border border-[var(--admin-border)] rounded text-[11px] sm:text-[11px]">Esc</span>
            <span>Back</span>
          </div>
        </div>
      </div>
      ...
      <motion.div variants={fadeUp} className="bg-[#f8fafc] border border-[var(--admin-accent)]/20 rounded-[var(--admin-radius-lg)] p-4 flex items-start gap-3 shadow-sm mb-6 hidden lg:flex">
        <span className="material-symbols-outlined text-[var(--admin-accent)] mt-0.5 text-[20px]">lightbulb</span>
        <div>
          ...
          <p className="text-[11px] text-[var(--admin-text-secondary)] mt-1 leading-relaxed max-w-[800px]">
            Upload an image and use the <strong>AI Vision</strong> tool to automatically generate showcase titles, descriptions, and categories. Or use <kbd className="px-1.5 py-0.5 bg-[var(--admin-surface-muted)] border border-[var(--admin-border)] rounded">Alt + →</kbd> to quickly navigate through the form steps.
          </p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 items-start">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {currentStep === 1 && (
                  <div className="space-y-5">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div>
                        <h2 className="text-[11px] font-bold text-[var(--admin-text-primary)]">Showcase Specifications</h2>
                        <p className="text-[11px] text-[var(--admin-text-secondary)]">Give your arrangement a title, short subtitle, and catalog category.</p>
                      </div>
                      <button
                        type="button"
                ...
                {currentStep === 2 && (
                  <div className="space-y-5">
                    <div>
                      <h2 className="text-[11px] font-bold text-[var(--admin-text-primary)]">Commercials & Aesthetics</h2>
                      <p className="text-[11px] text-[var(--admin-text-secondary)]">Define rental rates, colors, setup time, and prop lists.</p>
                    </div>
                ...
                {currentStep === 3 && (
                  <div className="space-y-5">
                    <div>
                      <h2 className="text-[11px] font-bold text-[var(--admin-text-primary)]">Atmospheric Narrative</h2>
                      <p className="text-[11px] text-[var(--admin-text-secondary)]">Provide clients with rich heritage descriptions, aesthetics, and setup context.</p>
                    </div>
                      ...
                      <div className="col-span-1 sm:col-span-2 p-5 bg-[var(--admin-bg-subtle)] border border-[var(--admin-border)] rounded-2xl space-y-4 text-[12px]">
                        ...
                        <div className="space-y-3">
                          <div className="flex flex-col sm:flex-row sm:justify-between border-b border-[var(--admin-border)]/40 pb-2 gap-1">
                            <span className="font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider text-[11px]">Showcase Title</span>
                            <span className="font-bold text-[var(--admin-text-primary)] sm:text-right">{formData.title || "Unassigned"}</span>
                          </div>
                          {formData.subtitle && (
                            <div className="flex flex-col sm:flex-row sm:justify-between border-b border-[var(--admin-border)]/40 pb-2 gap-1">
                              <span className="font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider text-[11px]">Subtitle</span>
                              <span className="font-semibold text-[var(--admin-text-primary)] sm:text-right">{formData.subtitle}</span>
                            </div>
                          )}
                          <div className="flex flex-col sm:flex-row sm:justify-between border-b border-[var(--admin-border)]/40 pb-2 gap-1">
                          ...
                          <div className="flex flex-col sm:flex-row sm:justify-between border-b border-[var(--admin-border)]/40 pb-2 gap-1">
                            <span className="font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider text-[11px]">Rental Price</span>
                            <span className="font-bold text-[var(--admin-text-primary)] sm:text-right">₹{Number(formData.rentalPrice || 0).toLocaleString()}</span>
                          </div>
                          <div className="flex flex-col sm:flex-row sm:justify-between border-b border-[var(--admin-border)]/40 pb-2 gap-1">
                            <span className="font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider text-[11px]">Colors</span>
                            <span className="font-bold text-[var(--admin-text-primary)] sm:text-right">{colors.length} mapped</span>
                          </div>
                          <div className="flex flex-col sm:flex-row sm:justify-between pb-1 gap-1">
                            <span className="font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider text-[11px]">Setup Time</span>
                            <span className="font-bold text-[var(--admin-text-primary)] sm:text-right">{formData.setupTimeHours} Hours</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className={`lg:sticky lg:top-24 space-y-6 w-full ${mobileTab === "preview" ? "block" : "hidden lg:block"}`}>
          <div className="text-center lg:text-left">
            ...
            <p className="text-[11px] sm:text-[11px] text-[var(--admin-text-secondary)]/75 mt-0.5">Real-time catalog rendition of your showcase design</p>
          </div>
            ...
            <div className="aspect-[4/3] bg-[var(--admin-bg-subtle)] relative overflow-hidden">
              {formData.image ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-[var(--admin-text-secondary)]/40">
                  ...
                  <span className="text-[11px] sm:text-[11px] font-bold uppercase tracking-widest">Image Preview Canvas</span>
                </div>
              )}
            </div>
              ...
              {colors.length > 0 && (
                <div className="flex items-center gap-1 mt-2">
                  {colors.slice(0, 4).map((c, idx) => (
                    <span key={idx} className="w-4 h-4 rounded-full border border-[var(--admin-border)] shadow-sm shrink-0" style={{ backgroundColor: c }} title={c} />
                  ))}
                </div>
              )}

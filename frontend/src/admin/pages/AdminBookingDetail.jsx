...
import { PageHeader, fadeUp, stagger, SkeletonDashboard } from "../components/AdminUIKit";
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (bookingRes.success) {
          const allBookings = Array.isArray(bookingRes.data) ? bookingRes.data : bookingRes.data?.data || [];
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={stagger} className="max-w-[1280px] mx-auto space-y-6 pb-20 p-4 sm:p-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-[20px] font-bold text-[var(--admin-text-primary)]">{selectedBooking.title}</h3>
            <p className="text-[12px] text-[var(--admin-text-secondary)] mt-1">Customer: {selectedBooking.user?.name} | {selectedBooking.user?.phone}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div variants={fadeUp} className="lg:col-span-2 space-y-6">
          <div className="admin-card p-6 md:p-8 space-y-8">
            ...
            <div className="space-y-4 pt-5 border-t border-[var(--admin-border-subtle)]">
              ...
              <button onClick={handleUpdateQuotation} className="admin-btn admin-btn-primary w-full h-10">Save & Send Price Estimate</button>
            </div>
        ...
        <motion.div variants={fadeUp} className="bg-[var(--admin-bg-subtle)] border border-[var(--admin-border-subtle)] rounded-[var(--admin-radius-xl)] p-6 flex flex-col h-[600px] sticky top-24">

...
export function AdminDashboard() {
  ...
  const quickActions = [
    { icon: "edit_note", label: "Edit Web Pages", path: "/admin/content", color: "var(--admin-accent)" },
    { icon: "design_services", label: "Custom Orders", path: "/admin/custom-orders", color: "var(--admin-info)" },
    ...
    <motion.div initial="hidden" animate="show" variants={stagger} className="space-y-6">
      ...
      <div className="admin-grid-charts">
        ...
        <ChartCard
          ...
          legend={
            <>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--admin-accent)]" />Sales</span>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[var(--admin-border-strong)]" />Orders</span>
            </>
          }
        >
          {orders.length === 0 ? (
            <div className="h-[280px] flex flex-col items-center justify-center bg-[var(--admin-bg-subtle)] rounded-[var(--admin-radius-lg)] border border-dashed border-[var(--admin-border)]">
              <span className="material-symbols-outlined text-[32px] text-[var(--admin-text-tertiary)] mb-2">analytics</span>
              <span className="text-[11px] uppercase font-bold text-[var(--admin-text-secondary)] tracking-wider">No Sales Trends Recorded</span>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={isMobile ? 220 : 280}>
              <AreaChart data={revenueChartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="month" tick={{ fontSize: 10, fill: "var(--admin-text-tertiary)" }} axisLine={false} tickLine={false} />
                <YAxis hide={isMobile} tick={{ fontSize: 10, fill: "var(--admin-text-tertiary)" }} axisLine={false} tickLine={false} tickFormatter={formatCurrency} />
                <Tooltip content={<ChartTooltip />} />
                <Area type="monotone" dataKey="revenue" stroke="var(--admin-accent)" fill="url(#colorSales)" strokeWidth={2} name="Sales" />
                <Area type="monotone" dataKey="orders" stroke="var(--admin-border-strong)" fill="transparent" strokeWidth={1.5} strokeDasharray="4 4" name="Orders" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
        ...
        <ChartCard title="Top Categories" subtitle="Sales distribution by category">
          {categoryChartData.length === 0 ? (
            <div className="h-[240px] flex flex-col items-center justify-center bg-[var(--admin-bg-subtle)] rounded-[var(--admin-radius-lg)] border border-dashed border-[var(--admin-border)]">
              ...
              <span className="text-[11px] uppercase font-bold text-[var(--admin-text-secondary)] tracking-wider">No Sales Recorded</span>
            </div>
          ) : (
            <>
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={categoryChartData} cx="50%" cy="50%" innerRadius={55} outerRadius={80} dataKey="value" paddingAngle={2} strokeWidth={0}>
                ...
                {categoryChartData.slice(0, 4).map((cat, i) => (
                  <div key={i} className="flex items-center justify-between text-[11px]">
                    <span className="flex items-center gap-2">
                      ...
                      <span className="text-[var(--admin-text-secondary)] font-medium truncate max-w-[120px]">{cat.name}</span>
                    </span>
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              {outOfStock > 0 && (
                <div className="flex items-center gap-3 p-3 rounded-[var(--admin-radius-md)] bg-[var(--admin-error-light)] border border-[var(--admin-error-border)] mb-3">
                  <span className="material-symbols-outlined text-[18px] text-[var(--admin-error)]">error</span>
                  <div>
                    <p className="text-[12px] font-bold text-[var(--admin-error)]">{outOfStock} Product{outOfStock > 1 ? "s" : ""} Out of Stock</p>
                    <p className="text-[10px] text-[var(--admin-error)] opacity-80 mt-0.5">Immediate attention required</p>
                  </div>
                </div>
              )}
              {lowStockProducts > 0 && (
                <div className="flex items-center gap-3 p-3 rounded-[var(--admin-radius-md)] bg-[var(--admin-warning-light)] border border-[var(--admin-warning-border)] mb-3">
                  <span className="material-symbols-outlined text-[18px] text-[var(--admin-warning)]">warning</span>
                  <div>
                    <p className="text-[12px] font-bold text-[var(--admin-warning)]">{lowStockProducts} Product{lowStockProducts > 1 ? "s" : ""} Low Stock</p>
                    <p className="text-[10px] text-[var(--admin-warning)] opacity-80 mt-0.5">Running below threshold</p>
                  </div>
                </div>
              )}
        ...
        <motion.div variants={fadeUp} className="admin-card p-4 sm:p-6 flex flex-col">
          ...
          {orders.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center bg-[var(--admin-bg-subtle)] rounded-[var(--admin-radius-lg)] border border-dashed border-[var(--admin-border)] p-4 text-center">
              <span className="material-symbols-outlined text-[28px] text-[var(--admin-text-tertiary)] mb-2">receipt_long</span>

          ...
          {layouts.map((layout) => (
            <motion.div key={layout._id} variants={fadeUp} className="admin-card overflow-hidden">
              <div className="flex items-center justify-between p-5 border-b border-[var(--admin-border-subtle)]">
                <div>
                  <h3 className="text-[15px] font-bold text-[var(--admin-text-primary)] leading-tight">{layout.name}</h3>
              ...
              <div className="divide-y divide-[var(--admin-border-subtle)]">
                {layout.sections.sort((a, b) => a.order - b.order).map((section, idx) => (
                  <div key={idx} className="flex items-center justify-between p-4 hover:bg-[var(--admin-surface-hover)] transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="font-semibold text-[13px] text-[var(--admin-text-primary)]">{section.componentName}</span>
                        {section.props?.title && (
                          <span className="block text-[11px] text-[var(--admin-text-tertiary)] mt-0.5">"{section.props.title}"</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-4 shrink-0">
                      <span className="text-[10px] font-bold text-[var(--admin-text-tertiary)] uppercase tracking-wider hidden sm:block">Visible</span>
                      <AdminToggle

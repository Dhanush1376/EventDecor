    ...
    >
        ...
        <div className="h-[350px] w-full">
          {!stats.monthlyRevenue || stats.monthlyRevenue.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center bg-[var(--admin-bg-subtle)] rounded-[var(--admin-radius-lg)] border border-dashed border-[var(--admin-border)]">
              ...
              <span className="text-[11px] uppercase font-bold text-[var(--admin-text-secondary)] tracking-wider">No Revenue Data</span>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={[...stats.monthlyRevenue].reverse()} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                <defs>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-[300px]">
            {!stats.categoryPerformance || stats.categoryPerformance.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center bg-[var(--admin-bg-subtle)] rounded-[var(--admin-radius-lg)] border border-dashed border-[var(--admin-border)]">
                ...
                <span className="text-[11px] uppercase font-bold text-[var(--admin-text-secondary)] tracking-wider">No Category Data</span>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
        ...
        <motion.div variants={fadeUp} className="admin-card p-6 flex flex-col">
          ...
          <p className="text-[11px] text-[var(--admin-text-tertiary)] mb-6">Latest events from your store</p>
                  ...
                  <div key={i} className="flex gap-4 items-start">
                    ...
                    <div>
                      <p className="text-[12.5px] text-[var(--admin-text-primary)] leading-snug">
                        <span className="font-semibold text-[var(--admin-text-primary)]">{activity.user}</span> {activity.action}
                      </p>

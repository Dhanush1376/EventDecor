    ...
    >
      ...
      >
        ...
        <button
          ...
          className="px-4 py-2.5 text-[11px] sm:text-[11px] font-bold uppercase tracking-wider bg-black text-white hover:bg-slate-900 rounded-lg shadow-xs flex items-center gap-1.5 transition-all group"
        >
      ...
      >
        <div className="grid grid-cols-2 md:flex md:items-center gap-3 w-full md:w-auto">
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-[11px] text-slate-700 font-medium py-2 outline-none cursor-pointer w-full md:min-w-[180px] shadow-xs"
          >
          ...
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-[11px] text-slate-700 font-medium py-2 outline-none cursor-pointer w-full shadow-xs"
          >
        ...
        {selectedProducts.length > 0 && (
          <span className="text-[11px] sm:text-[11px] font-bold text-slate-900 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-full shrink-0">
      ...
      {filteredProducts.length === 0 ? (
        <div className="py-20 text-center bg-white rounded-xl border border-slate-200/80 flex flex-col items-center justify-center">
          ...
          <p className="text-[11px] sm:text-[11px] font-bold text-slate-700 mt-2">Data Not Found</p>
          <p className="text-[11px] text-slate-450 mt-2">No products matched your active filters or search terms.</p>
        </div>
      ) : viewMode ==="table" ? (
        <motion.div
          variants={fadeUp}
                  ...
                  >
                    ...
                    <td className="p-4 text-left">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                          <p className="font-semibold text-slate-800 text-[11px] sm:text-[11px] group-hover:text-black transition-colors">
                            {p.name}
                          </p>
                    ...
                    <td className="p-4 text-left">
                      <span
                        className={`px-2 py-0.5 rounded border text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 ${statusColors[p.status] ||"text-slate-500 bg-slate-50 border-slate-200"}`}
                      >
          ...
          {filteredProducts.map((p) => (
            ...
            >
              <div className="relative aspect-square overflow-hidden bg-slate-50 shrink-0">
                ...
                />
                {p.featured && (
                  <div className="absolute top-2 left-2 bg-black text-white px-2 py-0.5 rounded text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 shadow-xs">
                    Featured
                  </div>
                )}
                <span
                  className={`absolute top-2 right-2 px-2 py-0.5 rounded text-[11px] font-bold border px-2.5 py-1 ${statusColors[p.status]}`}
                >
              ...
              <div className="p-3.5 flex-1 flex flex-col justify-between">
                <div>
                  <p className="text-[11px] font-bold text-slate-800 truncate group-hover:text-black transition-colors">
                    {p.name}
                  </p>
                  <p className="text-[11px] text-slate-450 mt-1 mt-0.5">{p.category}</p>
                </div>
                <div className="flex items-center justify-between mt-3.5 pt-2 border-t border-slate-100">
                  <span className="text-[11px] font-bold text-black">
                    ₹{p.price.toLocaleString()}
                  </span>
                  <span className={`text-[11px] font-medium ${p.stock <= 5 ?"text-amber-600 font-bold" :"text-slate-450"}`}>
                    {p.stock} left
                  </span>
                </div>
              </div>

...
import { PageHeader, StatusBadge, fadeUp, stagger } from "../components/AdminUIKit";
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={stagger} className="space-y-6">
      <PageHeader
        ...
        subtitle={isEditing ? "Update showcase assets and details" : "Upload design inspiration or real event details"}
        ...
        <div className="lg:col-span-7 space-y-4 sm:space-y-6">
          <div className="admin-card p-5 sm:p-6 space-y-5">
            ...
            <div className="space-y-1">
              ...
              <textarea value={newItem.description} onChange={(e) => setNewItem({ ...newItem, description: e.target.value })} className="admin-textarea" rows={3} placeholder="Brief design concept..." />
            </div>
            <div className="space-y-1">
              <label className="admin-label">Story & Crafting Details (Optional)</label>
              <textarea value={newItem.story} onChange={(e) => setNewItem({ ...newItem, story: e.target.value })} className="admin-textarea" rows={3} placeholder="Studio story or floral crafting journey..." />
            </div>
          </div>

          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => navigate("/admin/gallery")} className="admin-btn admin-btn-outline px-6 py-2.5">
              Cancel
            </button>
            <button type="submit" disabled={submitting} className="admin-btn admin-btn-primary px-8 py-2.5">

  ...
  useEffect(() => {
    const fetchCoupons = async () => {
      try {
        ...
        if (res?.data) {
          const activeCoupons = res.data.filter(c => c.isActive && new Date(c.expiryDate) > new Date());
      ...
      <div className="max-w-[1200px] mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-[var(--color-text-secondary)] max-w-2xl mx-auto">
            Explore our latest active coupons and apply them at checkout for special discounts on your event orders.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-neutral-100 shadow-sm">
            <span className="material-symbols-outlined text-5xl text-neutral-300 mb-4 block">local_offer</span>
            <h3 className="text-xl font-bold text-[var(--color-text-primary)] mb-2">No active offers right now</h3>
            <p className="text-[var(--color-text-secondary)]">Please check back later for new promotions and discounts.</p>
          </div>
        ) : (
          <motion.div 

...
export function Dashboard() {
  ...
  const fetchRecentlyViewedList = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch recently viewed list:", err);
    } finally {
            ...
            >
              ...
              <div className="flex flex-col gap-1.5 p-4 text-on-surface-variant/40 font-label-sm text-[10px] uppercase tracking-widest font-bold border-t border-surface-container">
                <span className="max-w-md">
                  Studio Headquarters: Siri Arts & Crafts, Heritage Block,
                  Jayanagar, Bengaluru - 560041
                </span>
                <span>Inquiry Line: +91 98765 43210</span>
              </div>
            </div>
          </div>
                            ...
                            <div className="flex flex-col sm:flex-row justify-between gap-4 pb-3 border-b border-surface-container">
                             ...
                             <div className="pt-3 space-y-4">
                              {order.items?.map((item, itemIdx) => {
                                ...
                                const prodImage = item.imageSrc || (typeof item.productId === "object" ? item.productId?.imageSrc || item.productId?.images?.[0] : null) || "https://images.unsplash.com/photo-1543083477-4f785aeafaa9?auto=format&fit=crop&w=600&q=80";
            ...
            >
              ...
              <div className="print-invoice-area p-8 text-black text-sm bg-white font-sans relative">
                ...
                <div className="no-print flex justify-between items-center pb-4 mb-6 border-b border-gray-100">
                  <h3 className="font-display text-md font-bold uppercase tracking-wider text-primary">
                    Tax Invoice Preview
                  </h3>
                  <div className="flex gap-2">
                    <button
                      onClick={() => window.print()}
                      className="px-4 py-2 bg-primary text-white font-bold text-[10px] uppercase tracking-widest rounded-xl hover:brightness-110 shadow-sm cursor-pointer flex items-center gap-1.5"
                    >
                ...
                <div className="flex justify-between items-start border-b-2 border-gray-900 pb-6 mb-6">
                  <div>
                    <h1 className="text-3xl font-display font-black uppercase tracking-widest text-[#735c00]">Siri Arts</h1>
                    ...
                    <div className="text-[11px] text-gray-600 mt-2 space-y-0.5 leading-relaxed font-light">
                      <p>Siri Arts & Crafts Studio</p>
                      <p>Heritage Block, Jayanagar</p>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[11px] text-gray-600 mt-3 space-y-1">
                      <p>Invoice No: <strong className="text-black font-mono">{selectedInvoiceOrder.invoiceNumber || `INV-${selectedInvoiceOrder._id.substring(selectedInvoiceOrder._id.length - 8).toUpperCase()}`}</strong></p>
                      <p>Order Reference: <span className="font-mono">{selectedInvoiceOrder._id.substring(0, 12)}...</span></p>
                      <p>Invoice Date: {new Date(selectedInvoiceOrder.createdAt).toLocaleDateString("en-IN", { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                      <p>Payment Mode: <strong className="text-black uppercase">{selectedInvoiceOrder.paymentMethod || selectedInvoiceOrder.payment}</strong></p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8 mb-8 text-[12px]">
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 print:bg-white print:border-none print:p-0">
                    ...
                    <p className="font-bold text-gray-900 text-xs">{selectedInvoiceOrder.shippingAddress?.name || user?.name}</p>
                    <p className="text-gray-600 mt-1">{user?.email}</p>
                    <p className="text-gray-600 mt-0.5">{selectedInvoiceOrder.shippingAddress?.phone || user?.phone}</p>
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 print:bg-white print:border-none print:p-0">
                    ...
                    <p className="text-gray-900 font-semibold">{selectedInvoiceOrder.shippingAddress?.name || user?.name}</p>
                    <p className="text-gray-600 mt-1 leading-relaxed">
                      {selectedInvoiceOrder.shippingAddress?.address || selectedInvoiceOrder.address}, {selectedInvoiceOrder.shippingAddress?.locality || ""},
                      <br />
                      {selectedInvoiceOrder.shippingAddress?.city || ""}, {selectedInvoiceOrder.shippingAddress?.state || ""} — <strong className="text-black font-bold">{selectedInvoiceOrder.shippingAddress?.pincode || ""}</strong>
                    </p>
                    {selectedInvoiceOrder.shippingAddress?.landmark && (
                      <p className="text-gray-500 text-[11px] mt-1">Landmark: {selectedInvoiceOrder.shippingAddress.landmark}</p>
                    )}
                  </div>
                </div>
                ...
                <table className="w-full mb-8 border-collapse text-[12px]">
                  ...
                  <tfoot className="border-t border-gray-300">
                    <tr>
                      ...
                      <td className="text-right p-2 font-bold font-mono text-gray-900">₹{(selectedInvoiceOrder.total - (selectedInvoiceOrder.shippingFee || 0) + (selectedInvoiceOrder.discount || 0)).toLocaleString()}</td>
                    </tr>
                    {selectedInvoiceOrder.discount > 0 && (
                      <tr>
                        <td colSpan="5" className="text-right p-2 font-bold text-green-700">Coupon Discount:</td>
                        <td className="text-right p-2 font-bold font-mono text-green-700">-₹{selectedInvoiceOrder.discount.toLocaleString()}</td>
                      </tr>
                    )}
                    {selectedInvoiceOrder.shippingFee > 0 && (
                      <tr>
                        <td colSpan="5" className="text-right p-2 font-bold text-gray-600">Bespoke Shipping Fee:</td>
                        <td className="text-right p-2 font-bold font-mono text-gray-900">₹{selectedInvoiceOrder.shippingFee.toLocaleString()}</td>
                      </tr>
                    )}
                    <tr className="bg-gray-50 print:bg-white border-t border-double border-gray-900">
                      ...
                      <td className="text-right p-3 font-black font-mono text-[#735c00] text-[15px]">₹{selectedInvoiceOrder.total.toLocaleString()}</td>
                    </tr>
                  </tfoot>
                </table>
                  ...
                  <table className="w-full text-[10px]">
                    <tbody>
                      <tr>
                        ...
                        <td className="text-right py-1 font-mono text-gray-900 font-semibold">₹{(selectedInvoiceOrder.total / 1.18).toFixed(2)}</td>
                      </tr>
                      <tr>
                        <td className="py-1 text-gray-600">Integrated SGST (9%):</td>
                        <td className="text-right py-1 font-mono text-gray-900 font-semibold">₹{((selectedInvoiceOrder.total - selectedInvoiceOrder.total / 1.18) / 2).toFixed(2)}</td>
                      </tr>
                      <tr>
                      ...
                      <tr className="border-t border-dashed border-gray-300 font-bold font-mono">
                        ...
                        <td className="text-right pt-2 text-[#735c00] font-black">₹{(selectedInvoiceOrder.total - selectedInvoiceOrder.total / 1.18).toFixed(2)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                ...
                <div className="flex justify-between items-start border-t border-gray-300 pt-6 mt-8">
                  <div className="w-1/2 space-y-1">
                    ...
                    <p className="text-[11px] text-gray-600">Logistics Carrier: <strong className="text-black font-semibold">{selectedInvoiceOrder.courierPartner || "Delhivery Logistics"}</strong></p>
                    <p className="text-[11px] text-gray-600">Tracking AWB: <strong className="text-black font-mono font-bold">{selectedInvoiceOrder.trackingNumber || `SR-${selectedInvoiceOrder._id.substring(selectedInvoiceOrder._id.length - 8).toUpperCase()}-IN`}</strong></p>
                    <div className="pt-3">
                      <Barcode value={selectedInvoiceOrder.trackingNumber || `SR-${selectedInvoiceOrder._id.substring(selectedInvoiceOrder._id.length - 8).toUpperCase()}-IN`} height={35} width={1.2} displayValue={true} fontSize={9} margin={0} />
                    </div>
                  </div>
                  
                  <div className="flex flex-col items-center">
                    <h3 className="font-black text-gray-800 text-[10px] uppercase tracking-wider mb-2">Scan Live Transit</h3>
                    <QRCodeSVG value={`${window.location.origin}/track/${selectedInvoiceOrder._id || selectedInvoiceOrder.id}`} size={90} level="M" />
                    <span className="text-[8px] text-gray-400 mt-2 font-light font-mono">Powered by Delhivery Feed</span>
                  </div>
                </div>

                {/* Footer Disclaimer */}
                <div className="mt-12 text-center text-gray-400 text-[10px] border-t border-gray-100 pt-4 leading-normal font-light">
                  This is a secure computer generated tax invoice issued under Siri Arts & Crafts Jayanagar boutique regulations and requires no physical signatures. For inquiry, reach support@siriartsandcrafts.com.
                </div>
              </div>
            </motion.div>
          </>

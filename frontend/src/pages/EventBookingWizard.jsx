...
export function EventBookingWizard() {
  ...
  const simulateAiAnalysis = () => {
    ...
    setTimeout(() => {
      ...
      setAiAnalysisResult({
        ...
        recommendedInclusions: "Vedic Backdrop Panels, Ambient Amber Up-lighting & Royal Entrance Swings",
  ...
  return (
    <div className="bg-[#fcfbf9] min-h-screen text-on-surface pt-20 md:pt-32 pb-24 relative overflow-hidden font-body">
      <SEO title="Luxury Event Decor Booking | Guided Studio Wizard" description="Configure and blueprint your milestone ceremonies with our guided luxury booking wizard." />

      <MandalaArtDecor variant={3} size={500} className="-top-32 -right-32 absolute opacity-[0.08]" spinDuration={200} />
      <MandalaArtDecor variant={1} size={400} className="-bottom-20 -left-20 absolute opacity-[0.06]" spinDuration={180} />
        ...
        <div className="bg-white rounded-[24px] md:rounded-[40px] border border-black/5 shadow-2xl p-6 md:p-12 min-h-[480px] flex flex-col justify-between">
          <AnimatePresence mode="wait">
            ...
            {currentStep === 1 && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-8">
                <div className="space-y-2">
                  <h3 className="font-display text-[20px] md:text-[28px] text-black font-semibold">Select Your Occasion</h3>
                  <p className="font-body text-black/45 text-[12px] md:text-[13px]">Select the classification of your milestone celebration to load specialized structural presets.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {EVENT_TYPES.map((type) => (
                    ...
                    >
                      <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center shrink-0 transition-colors ${
                        formData.eventType === type.id ? "bg-primary text-white" : "bg-primary/5 text-primary group-hover:bg-primary group-hover:text-white"
                      }`}>
                        <span className="material-symbols-outlined text-[22px]">{type.icon}</span>
                      </div>
                      <div className="space-y-1">
                        <h4 className="font-display text-[16px] text-black font-bold group-hover:text-primary transition-colors">{type.label}</h4>
                        <p className="font-body text-black/40 text-[11px] leading-relaxed font-light">{type.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {currentStep === 2 && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-8">
                <div className="space-y-2">
                  <h3 className="font-display text-[20px] md:text-[28px] text-black font-semibold">Select Event Design</h3>
                  <p className="font-body text-black/45 text-[12px] md:text-[13px]">Select an existing package to load designs, or select "Custom Setup" below.</p>
                </div>
                ...
                {loading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    >
                      <div>
                        <span className="bg-stone-200 text-stone-700 px-2 py-0.5 rounded-full font-label text-[8px] uppercase tracking-widest font-bold">Custom Blueprint</span>
                        <h4 className="font-display text-[18px] text-black font-bold mt-3">Bespoke Architectural Custom Setup</h4>
                        <p className="font-body text-black/40 text-[11px] leading-relaxed mt-2 font-light">Collaborate directly with our design artisans to build a completely unique visual space from scratch.</p>
                      </div>
                      <span className="font-label text-[10px] text-primary uppercase font-bold tracking-widest">Map custom layout →</span>
                    </div>
                      ...
                      .map((pkg) => (
                        <div
                          ...
                          className={`p-4 rounded-[24px] border text-left cursor-pointer transition-all flex gap-4 items-center ${
                            formData.eventPackageId === (pkg._id || pkg.id) ? "bg-primary/5 border-primary/40 shadow-lg" : "border-black/5 hover:border-black/15"
                          }`}
                        >
            ...
            {currentStep === 3 && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-8">
                <div className="space-y-2">
                  <h3 className="font-display text-[20px] md:text-[28px] text-black font-semibold">Logistics & Schedule Details</h3>
                  <p className="font-body text-black/45 text-[12px] md:text-[13px]">Provide event timestamps and spatial specifications to verify structural clearances.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Event Title */}
                  <div className="space-y-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Event Booking Title</label>
                    <input
                      type="text"
                  ...
                  {/* Event Date */}
                  <div className="space-y-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Event Date</label>
                    <input
                      type="date"
                      value={formData.date}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2">
                      <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Starts At</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-2">
                      <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Ends At</label>
                      <input
                        type="text"
                  ...
                  <div className="space-y-2 md:col-span-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Venue Address</label>
                    <textarea
                  ...
                  <div className="space-y-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Google Maps Link (Optional)</label>
                    <input
                      type="url"
                  ...
                  <div className="flex items-center gap-3 pt-6">
                    ...
                    />
                    <label htmlFor="isOutdoor" className="font-body text-[13px] text-black font-semibold cursor-pointer">This is an Outdoor Open-Air Lawn Ceremony</label>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {currentStep === 4 && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-8">
                <div className="space-y-2">
                  <h3 className="font-display text-[20px] md:text-[28px] text-black font-semibold">Extra Decorations & Setup Options</h3>
                  <p className="font-body text-black/45 text-[12px] md:text-[13px]">Define color swatches and lighting profiles or add standalone prop rentals.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Theme Colors */}
                  <div className="space-y-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Color Palette Preference</label>
                    <input
                      type="text"
                  ...
                  {/* Floral preferences */}
                  <div className="space-y-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Floral Garlands Preference</label>
                    <input
                      type="text"
                      value={formData.customization.floralPreference}
                      onChange={(e) => handleNestedInputChange("customization", "floralPreference", e.target.value)}
                      className="w-full px-5 py-3 rounded-full border border-black/5 bg-[#fbf9f6] text-[13px] outline-none focus:border-primary/45 transition-colors"
                    />
                  </div>
                  ...
                  {/* Lighting profile */}
                  <div className="space-y-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Lighting Profile</label>
                    <input
                      type="text"
                      value={formData.customization.lightingPreference}
                      onChange={(e) => handleNestedInputChange("customization", "lightingPreference", e.target.value)}
                      className="w-full px-5 py-3 rounded-full border border-black/5 bg-[#fbf9f6] text-[13px] outline-none focus:border-primary/45 transition-colors"
                    />
                  </div>
                  ...
                  {/* Stage dimensions */}
                  <div className="space-y-2">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Stage Size Dimensions</label>
                    <input
                      type="text"
                  ...
                  <div className="md:col-span-2 space-y-4 pt-4 border-t border-black/5">
                    <label className="font-label text-[10px] uppercase tracking-wider text-black/50 font-bold block">Add-on Visual Prop Rentals</label>
                        ...
                        return (
                          <div
                            ...
                            className={`p-4 rounded-xl border text-left cursor-pointer transition-all ${
                              isChecked ? "bg-primary/5 border-primary/30" : "border-black/5 hover:border-black/10"
                            }`}
                          >
                            <div className="flex justify-between items-start gap-2">
                              <span className="font-body text-[12px] text-black font-bold leading-tight">{addon.name}</span>
                              <input type="checkbox" checked={isChecked} readOnly className="accent-primary w-4 h-4 shrink-0" />
                            </div>
                            <span className="font-display text-xs text-black/50 block mt-2 font-semibold">+ ₹{addon.price.toLocaleString("en-IN")}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            {currentStep === 5 && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-8">
                <div className="space-y-2">
                  <h3 className="font-display text-[20px] md:text-[28px] text-black font-semibold">Custom Decor Recommendations & Moodboards</h3>
                  <p className="font-body text-black/45 text-[12px] md:text-[13px]">Upload your Pinterest moodboard, stage maps, or custom designs. Our AI model will automatically analyze details, map color palettes, and draft preliminary pricing ranges.</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                  ...
                  <div className="lg:col-span-5 space-y-4">
                    <div className="border-2 border-dashed border-black/10 rounded-[24px] p-8 text-center bg-[#fcfbf9] relative group hover:border-primary/30 transition-colors flex flex-col items-center justify-center min-h-[220px]">
                      <input type="file" onChange={handleFileUploadSim} className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" />
                      ...
                      <span className="font-body text-xs text-black font-bold block">Upload Pinterest Moodboard</span>
                      <span className="font-body text-[10px] text-black/30 mt-1 block">Supports PNG, JPG, or PDF blueprint up to 10MB</span>
                      {uploadedFileName && (
                      ...
                      {isAiAnalyzing ? (
                        <div className="space-y-4 text-center py-6">
                          ...
                          <div className="space-y-1.5">
                            <span className="font-label text-[9px] uppercase tracking-[0.25em] text-primary font-bold block animate-pulse">SIRI ARTS AI DESIGN ASSISTANT</span>
                            <p className="font-body text-black/50 text-xs">Analyzing spatial layers, garland structures, and color swatches...</p>
                          </div>
                        </div>
                      ) : aiAnalysisResult ? (
                        <div className="space-y-5 animate-fade-in">
                          <div className="flex justify-between items-center border-b border-black/5 pb-3">
                            <span className="font-label text-[9px] uppercase tracking-[0.3em] text-primary font-bold">Custom Decor Recommendations</span>
                            <span className="bg-[#8B0000]/10 text-[#8B0000] px-2 py-0.5 rounded-full font-mono text-[8px] uppercase tracking-widest font-bold">Matches catalog patterns</span>
                          </div>

                          <div className="grid grid-cols-2 gap-y-4 gap-x-4">
                            <div className="space-y-0.5">
                              <span className="font-label text-[8px] uppercase tracking-widest text-black/40 block">Detected Theme Style</span>
                              <span className="font-body text-xs text-black font-bold">{aiAnalysisResult.detectedOccasion}</span>
                            </div>
                            <div className="space-y-0.5">
                              <span className="font-label text-[8px] uppercase tracking-widest text-black/40 block">Atmospheric Mood</span>
                              <span className="font-body text-xs text-black font-semibold truncate block">{aiAnalysisResult.mood}</span>
                            </div>
                            ...
                            <div className="space-y-0.5">
                              <span className="font-label text-[8px] uppercase tracking-widest text-black/40 block">Artisan Logistics Estimate</span>
                              <span className="font-body text-xs text-stone-700 font-medium">{aiAnalysisResult.estimatedSetupTime}</span>
                            </div>
                            <div className="space-y-0.5">
                              <span className="font-label text-[8px] uppercase tracking-widest text-black/40 block">Estimated Budget Range</span>
                              <span className="font-display text-sm text-primary font-bold italic">{aiAnalysisResult.estimatedPriceRange}</span>
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="text-center py-8 space-y-3">
                          <span className="material-symbols-outlined text-[36px] text-black/10">upload_file</span>
                          <p className="font-body text-xs text-black/40 max-w-xs mx-auto">Upload an inspiration visual above to launch the AI catalog comparator and auto-populate customizations.</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {currentStep === 6 && (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-8">
                <div className="space-y-2">
                  <h3 className="font-display text-[20px] md:text-[28px] text-black font-semibold">Review Your Event Summary</h3>
                  <p className="font-body text-black/45 text-[12px] md:text-[13px]">Review your itemized package selection, logistics dates, and estimated initial milestone deposit.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                  ...
                  <div className="space-y-4 bg-stone-50 rounded-[20px] p-6 border border-black/5">
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="flex justify-between"><span className="text-black/45">Floral Garlands:</span><span className="text-black font-semibold text-right truncate max-w-[180px]">{formData.customization.floralPreference}</span></div>
                      <div className="flex justify-between"><span className="text-black/45">Color Palette:</span><span className="text-black font-semibold text-right truncate max-w-[180px]">{formData.customization.themeColor}</span></div>
                      <div className="flex justify-between"><span className="text-black/45">Lighting Profile:</span><span className="text-black font-semibold text-right truncate max-w-[180px]">{formData.customization.lightingPreference}</span></div>
                    </div>
                  </div>

                  {/* Right: Itemized pricing estimates */}
                  <div className="space-y-6 bg-[#fcfbf9] rounded-[20px] p-6 border border-black/5">
                    <h4 className="font-display text-base text-black font-bold border-b border-black/5 pb-2">Preliminary Estimate</h4>
                    <div className="space-y-3 text-xs">
                      <div className="flex justify-between"><span className="text-black/50">Decor Setup & Rental Fee:</span><span className="text-black font-semibold">₹{(formData.eventPackageId ? (activePackage ? parseInt(activePackage.pricing.replace(/[^0-9]/g, "")) || 35000 : 35000) : 25000).toLocaleString("en-IN")}</span></div>
                      {formData.selectedAddons?.map((addon) => (
                        <div key={addon.name} className="flex justify-between"><span className="text-black/50">+ {addon.name}:</span><span className="text-black font-semibold">₹{addon.price.toLocaleString("en-IN")}</span></div>
                      ))}
                      <div className="flex justify-between"><span className="text-black/50">+ Transportation & Setup:</span><span className="text-primary font-bold tracking-wider uppercase">Calculated after admin review</span></div>
          ...
          <div className="flex items-center justify-between pt-10 border-t border-black/5 mt-10">
            <button
              ...
              className={`px-8 py-3 rounded-full font-label text-[10px] uppercase tracking-widest font-bold transition-all border ${
                currentStep === 1 ? "border-transparent text-black/10 cursor-not-allowed" : "border-stone-950/20 text-stone-700 hover:bg-stone-50"
              }`}
            >
              Back
            </button>

...
export function CustomizerDrawer({
  ...
  if (!selectedShowcase || typeof document === 'undefined') return null;
      ...
      {selectedShowcase && (
        <div className="fixed inset-0 z-50 pointer-events-none">
          ...
          <motion.div
            ...
            {...dragProps}
            className="fixed z-10 pointer-events-auto sm:inset-y-0 sm:top-0 sm:bottom-0 sm:right-0 sm:left-auto sm:w-[620px] sm:h-full sm:max-h-none sm:rounded-none bottom-0 inset-x-0 max-h-[90dvh] h-auto rounded-t-3xl bg-white shadow-2xl flex flex-col overflow-hidden"
          >
            {isMobile && <DrawerDragHandle onClick={() => setSelectedShowcase(null)} />}
            <div className="flex-1 overflow-y-auto overscroll-contain touch-pan-y flex flex-col">
                ...
                <div className="space-y-2">
                  ...
                  {uploadedReferenceUrl && (
                    <span className="font-mono text-[10px] text-[#8B0000] block font-semibold">
                      <Check className="w-3.5 h-3.5 mr-1 inline-block -mt-0.5" /> Linked
                      successfully!
                    </span>
                  )}
                </div>
            ...
            <div className="p-6 pt-4 pb-[calc(1.5rem+env(safe-area-inset-bottom,0px))] sm:pb-6 border-t border-black/5 bg-white flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 shrink-0">

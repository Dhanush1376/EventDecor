...
import React, { useState, useMemo, useEffect, useCallback, useRef, Profiler } from 'react';
...
const CountdownPromo = React.memo(
...
export function ProductListing() {
  ...
  useEffect(() => {
    ...
    const hasActiveFilters = s || (cat && cat !== 'All');
    if (hasActiveFilters && visualSearch.results) {
      visualSearch.reset();
    }
  ...
  const { _cartCount, _setIsCartOpen } = useCart();
    ...
    <Profiler id="ProductListing" onRender={logRenderMetrics}>
      <div className="bg-surface min-h-screen">
        ...
        {!(isMobile && searchParam) && (
          <section className="relative min-h-[320px] md:h-[45vh] lg:h-[70vh] flex items-center overflow-hidden bg-on-surface-variant">
            <motion.div
        ...
        {/* Promo Banner - Extracted to prevent re-renders */}
        <CountdownPromo

      ...
      <div className="print-invoice-area p-5 sm:p-6 md:p-8 text-black text-[9px] sm:text-xs md:text-sm bg-white font-sans relative shadow-sm print:shadow-none mx-auto w-full max-w-4xl">
        ...
        <div className="no-print flex justify-between items-center pb-3 md:pb-4 mb-4 md:mb-6 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            {onClose && (
              ...
              >
                <span className="material-symbols-outlined text-[16px] md:text-[20px] transition-transform duration-300 group-hover:rotate-90">close</span>
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex justify-between items-start border-b-2 border-gray-900 pb-4 md:pb-6 mb-4 md:mb-6">
          ...
          <div className="text-right w-[45%]">
            <h2 className="text-[12px] sm:text-lg md:text-2xl font-black uppercase tracking-wider text-gray-800">TAX INVOICE</h2>
            <div className="text-[7px] sm:text-[9px] md:text-[11px] text-gray-600 mt-1.5 md:mt-3 space-y-0.5 md:space-y-1">
              ...
              <p className="hidden sm:block">Order Ref: <span className="font-mono">{orderId.substring(0, 12)}...</span></p>
              <p>Invoice Date: {invoiceDate}</p>
              <p>Payment: <strong className="text-black uppercase">{paymentMode}</strong></p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-4 md:gap-8 mb-4 md:mb-8">
          <div className="bg-gray-50 p-2 md:p-4 rounded-xl border border-gray-100 print:bg-white print:border-none print:p-0">
            ...
            {customerEmail && <p className="text-gray-600 mt-0.5 md:mt-1 text-[8px] md:text-[11px] truncate">{customerEmail}</p>}
            {customerPhone && <p className="text-gray-600 mt-0.5 text-[8px] md:text-[11px]">{customerPhone}</p>}
          </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-100 border-b border-gray-300 font-bold text-[7px] md:text-[10px]">
                <th className="text-left p-1.5 md:p-3 uppercase tracking-wider text-gray-700">Item</th>
                <th className="text-center p-1.5 md:p-3 uppercase tracking-wider text-gray-700">Qty</th>
                <th className="text-right p-1.5 md:p-3 uppercase tracking-wider text-gray-700 hidden sm:table-cell">Price</th>
                <th className="text-right p-1.5 md:p-3 uppercase tracking-wider text-gray-700 hidden md:table-cell">CGST</th>
                <th className="text-right p-1.5 md:p-3 uppercase tracking-wider text-gray-700 hidden md:table-cell">SGST</th>
                <th className="text-right p-1.5 md:p-3 uppercase tracking-wider text-gray-700">Total</th>
              </tr>
            </thead>
                  ...
                  <tr key={idx} className="hover:bg-gray-50/50">
                    <td className="p-1.5 md:p-3 font-semibold text-gray-900">
                      ...
                      {item.variant && item.variant !== "Default" && (
                        <span className="block text-[6px] md:text-[9px] text-gray-500 font-light mt-0.5">Style: {item.variant}</span>
                      )}
                    </td>
                    <td className="p-1.5 md:p-3 text-center text-gray-800">{qty}</td>
                    <td className="p-1.5 md:p-3 text-right font-mono text-gray-600 hidden sm:table-cell">₹{basePrice.toFixed(2)}</td>
                    <td className="p-1.5 md:p-3 text-right font-mono text-gray-600 hidden md:table-cell">₹{(cgst * qty).toFixed(2)}</td>
                    <td className="p-1.5 md:p-3 text-right font-mono text-gray-600 hidden md:table-cell">₹{(sgst * qty).toFixed(2)}</td>
                    <td className="p-1.5 md:p-3 text-right font-bold font-mono text-gray-950">₹{lineTotal.toLocaleString()}</td>
                  </tr>
                );
              })}
            </tbody>
            <tfoot className="border-t border-gray-300 text-[8px] md:text-[11px]">
              <tr>
                <td className="text-right p-1.5 md:p-2 font-bold text-gray-600 sm:hidden" colSpan="2">Gross Subtotal:</td>
                <td className="text-right p-1.5 md:p-2 font-bold text-gray-600 hidden sm:table-cell md:hidden" colSpan="3">Gross Subtotal:</td>
                <td className="text-right p-1.5 md:p-2 font-bold text-gray-600 hidden md:table-cell" colSpan="5">Gross Subtotal:</td>
                <td className="text-right p-1.5 md:p-2 font-bold font-mono text-gray-900">₹{subtotal.toLocaleString()}</td>
              </tr>
              {discount > 0 && (
                <tr>
                  <td className="text-right p-1.5 md:p-2 font-bold text-green-700 sm:hidden" colSpan="2">Coupon Discount:</td>
                  <td className="text-right p-1.5 md:p-2 font-bold text-green-700 hidden sm:table-cell md:hidden" colSpan="3">Coupon Discount:</td>
                  <td className="text-right p-1.5 md:p-2 font-bold text-green-700 hidden md:table-cell" colSpan="5">Coupon Discount:</td>
                  <td className="text-right p-1.5 md:p-2 font-bold font-mono text-green-700">-₹{discount.toLocaleString()}</td>
                </tr>
              )}
              {shippingFee > 0 && (
                <tr>
                  <td className="text-right p-1.5 md:p-2 font-bold text-gray-600 sm:hidden" colSpan="2">Shipping:</td>
                  <td className="text-right p-1.5 md:p-2 font-bold text-gray-600 hidden sm:table-cell md:hidden" colSpan="3">Shipping:</td>
                  <td className="text-right p-1.5 md:p-2 font-bold text-gray-600 hidden md:table-cell" colSpan="5">Shipping:</td>
                  <td className="text-right p-1.5 md:p-2 font-bold font-mono text-gray-900">₹{shippingFee.toLocaleString()}</td>
                </tr>
              )}
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4">
            ...
            <table className="w-full text-[7px] md:text-[10px]">
              <tbody>
                <tr>
                  ...
                  <td className="text-right py-0.5 md:py-1 font-mono text-gray-900 font-semibold">₹{(total / 1.18).toFixed(2)}</td>
                </tr>
                <tr>
                  <td className="py-0.5 md:py-1 text-gray-600">Integrated SGST (9%):</td>
                  <td className="text-right py-0.5 md:py-1 font-mono text-gray-900 font-semibold">₹{((total - total / 1.18) / 2).toFixed(2)}</td>
                </tr>
                <tr>
          ...
          <div className="flex justify-between items-center w-full sm:w-[45%]">
            <div className="space-y-0.5 md:space-y-1">
              <h3 className="font-black text-gray-800 text-[7px] md:text-[10px] uppercase tracking-wider mb-1 md:mb-2">Dispatch Audit</h3>
              <p className="text-[7px] md:text-[11px] text-gray-600">Carrier: <strong className="text-black font-semibold">{courierPartner}</strong></p>
              <p className="text-[7px] md:text-[11px] text-gray-600">AWB: <strong className="text-black font-mono font-bold">{trackingNumber}</strong></p>
              <div className="pt-1 md:pt-3">
                <Barcode value={trackingNumber} height={20} width={1} displayValue={false} margin={0} />
              </div>
            </div>
            
            <div className="flex flex-col items-center">
              <h3 className="font-black text-gray-800 text-[6px] md:text-[9px] uppercase tracking-wider mb-1 md:mb-2 text-center">Scan</h3>

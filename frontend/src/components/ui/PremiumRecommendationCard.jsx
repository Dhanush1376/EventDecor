...
export const PremiumRecommendationCard = React.memo(function PremiumRecommendationCard({
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-5 animate-pulse w-full">
        <div className="aspect-[3/4] w-full bg-surface-container-high rounded-[20px] md:rounded-[24px] overflow-hidden" />
  ...
  const numericOldPrice = typeof oldPrice === "number" ? oldPrice : parseFloat(String(oldPrice).replace(/[^\d.-]/g, ""));
  ...
  return (
    <motion.div
      className="group relative flex flex-col cursor-pointer w-full focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-4 rounded-[20px] md:rounded-[24px]"
    ...
    >
      {/* Visual Canvas */}
      <div className="relative aspect-[3/4] overflow-hidden rounded-[20px] md:rounded-[24px] bg-[#FAF9F6] border border-black/5 shadow-sm transition-all duration-700 ease-out group-hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)]">
          ...
          <div className="flex flex-col gap-2">
            {discount > 0 && (
              <div className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-white text-stone-900 flex flex-col items-center justify-center shadow-md transform -rotate-12 border border-black/10 ring-1 ring-black/5">
                <span className="text-[11px] md:text-[12px] font-extrabold leading-none">{discount}%</span>
                <span className="text-[7px] md:text-[8px] tracking-widest uppercase mt-0.5 font-bold text-stone-500">Off</span>
              </div>
            )}
          </div>
          ...
          {isHovered && (
            ...
            >
              ...
              >
                ...
                ) : targetType === "event" ? "View Event" : targetType === "gallery" ? "View Inspiration" : "Add to Bag"}
              </button>
        ...
        <div className="md:hidden absolute bottom-3 right-3 z-20">
          ...
          >
            <span className="material-symbols-outlined text-[20px]">
              {added ? "check" : targetType === "event" || targetType === "gallery" ? "arrow_forward" : "add"}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="pt-5 pb-2 px-1 flex flex-col flex-1">
        ...
        {score > 0 && (
          <div className="flex items-center gap-1.5 mt-2 transition-opacity duration-300" aria-label={`Match score: ${Math.round(score * 100)}%`}>

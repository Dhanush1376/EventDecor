...
export function BlogCard({ post, featured = false }) {
  ...
  if (featured) {
    return (
      <Link to={`/blog/${post.slug}`} className="group block w-full relative rounded-3xl overflow-hidden shadow-xl shadow-black/10 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
  ...
  return (
    <Link to={`/blog/${post.slug}`} className="group flex flex-col h-full bg-surface/50 backdrop-blur-sm border border-outline-variant/30 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">

/**
 * Cloudinary-hosted placeholder images for CMS defaults and admin UI samples.
 * Do not use external hosts (e.g. Unsplash) in production-facing code paths.
 */
const CDN = 'https://res.cloudinary.com/drxgnnzeb/image/upload';

const img = (path: string, width = 800) =>
  `${CDN}/q_auto,f_webp,w_${width}/${path}`;

export const PLACEHOLDER_IMAGES = {
  heroBackground: img('v1779181764/event_decor_ecommerce/assets/event_decor_mobile%20hero%20background.png'),
  mandalaHero: img('v1779129367/event_decor_ecommerce/assets/event_decor_mandala_hero_art.png'),
  collectionWedding: img('v1779129318/event_decor_ecommerce/assets/event_decor_collection_wedding.png', 600),
  ...
  emptyCart: img('v1779129342/event_decor_ecommerce/assets/event_decor_empty_cart_illustration.jpg', 600),
} as const;

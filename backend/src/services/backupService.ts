...
export class BackupService {
  ...
  public async createJsonBackup(type: 'daily' | 'weekly' | 'monthly' = 'daily'): Promise<string> {
    ...
    const collectionsToBackup = [
      'products', 'users', 'orders', 'inventoryledgers', 
      'inventoryreservations', 'categories', 'reviews', 'customorders'
    ];
  ...
  public async pushToGithub(backupDir: string, type: string): Promise<void> {
    ...
    if (!owner || !repo || !token) {
      logger.warn('[BackupService] GITHUB_BACKUP_OWNER, REPO, or TOKEN missing. Skipping GitHub offsite backup.');
      return;
    }
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (!response.ok) {
          ...
          logger.error(`[BackupService] GitHub API Error for ${file}: ${response.status} - ${errData}`);
        } else {
  ...
  public async createMongoDump(type: 'daily' | 'weekly' | 'monthly' = 'daily'): Promise<void> {
    ...
    try {
      ...
      logger.warn(`[BackupService] mongodump failed: ${err.message}. Ensure MongoDB Database Tools are installed.`);
    }
  }
  ...
  public async createEmergencySnapshot(reason: string): Promise<string> {
    ...
    const collections = [
      'products', 'users', 'orders', 'inventoryledgers', 'customorders'
    ];

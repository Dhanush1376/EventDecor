...
const testRestore = async () => {
  ...
  try {
    ...
    const testUri = process.env.MONGO_URI!.replace('siri-arts-crafts', 'siri-arts-crafts-test-restore');
    ...
    const backupFiles = fs.readdirSync(backupDir).filter(f => f.startsWith('products_daily_export_')).sort().reverse();
    ...
    console.log(`📊 Found in backup: ${backupData.products.length} Products, ${backupData.categories.length} Categories, ${backupData.galleries.length} Galleries`);

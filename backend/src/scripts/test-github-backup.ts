...
const testGithubBackup = async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(tempDir, 'test-connection.json'), JSON.stringify({ message: 'GitHub API Connection Successful', timestamp: new Date() }, null, 2));
    ...
    console.log('✅ Test complete. Check your GitHub repository for a "test" folder containing "test-connection.json".');

...
async function runAudit() {
  ...
  try {
    ...
    if (securityLevel === 'high' || securityLevel === 'under_attack') {
      logger.warn(
        'WARNING: Security Level is too high for mobile users. Change to "medium"to reduce false positive JS challenges.',
      );
    }
    ...
    if (http3 !== 'on') {
      logger.warn(
        'WARNING: HTTP/3 is off. Enable it to drastically improve mobile connection stability across cell towers.',
      );
    }
    ...
    if (sslMode !== 'strict' && sslMode !== 'full') {
      logger.warn(
        'WARNING: SSL is not set to Full/Strict. This can cause redirect loops on mobile Safari.',
      );
    }
    ...
    logger.info(
      '� Ensure any custom firewall rules use "Managed Challenge"(Turnstile) instead of "JS Challenge".',
    );

    logger.info(
      '🎉 Audit Complete! Please adjust any flagged settings in your Cloudflare dashboard.',
    );
  } catch (err: any) {

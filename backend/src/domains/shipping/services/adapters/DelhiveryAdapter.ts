...
export class DelhiveryAdapter implements ICourierAdapter {
  ...
  private providerName = 'Delhivery';

  constructor() {
    this.httpClient = new CourierHttpClient(shippingConfig.delhivery.apiUrl);
  }

  private notImplemented(operation: string): never {
    throw new ApiError(
      501,
      `${this.providerName} ${operation} is not configured. Missing DELHIVERY_API_KEY in environment.`,
    );
  }

  private checkConfig(operation: string) {
    if (!shippingConfig.delhivery.isConfigured) {

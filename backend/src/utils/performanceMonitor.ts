...
export const startPerformanceMonitoring = () => {
  ...
  eventLoopMonitorInterval = setInterval(() => {
    ...
    if (lag > LAG_WARNING_THRESHOLD_MS) {
      logger.warn(
        `[PERF] Event loop lag detected: ${Math.round(lag)}ms (threshold: ${LAG_WARNING_THRESHOLD_MS}ms)`,
      );
    }
    ...
    if (heapRatio > MEMORY_WARNING_THRESHOLD_PERCENT) {
      logger.warn(
        `[PERF] High memory pressure: ${Math.round(heapRatio * 100)}% heap used (${Math.round(memUsage.heapUsed / 1024 / 1024)}MB / ${Math.round(maxHeap / 1024 / 1024)}MB)`,
      );
    }
  }, 5000);
...
export const slowQueryLoggerPlugin = (schema: mongoose.Schema) => {
  ...
  schema.pre(
    /^(find|findOne|findOneAndUpdate|findOneAndDelete|countDocuments|aggregate)$/,
  ...
  schema.post(
    /^(find|findOne|findOneAndUpdate|findOneAndDelete|countDocuments|aggregate)$/,
    function (this: any, res: any, next: any) {
      if (this._startTime) {
        ...
        if (duration > SLOW_QUERY_THRESHOLD_MS) {
          ...
          try {
            const filter =
              typeof this.getFilter === 'function' ? this.getFilter() : this._pipeline || [];
          ...
          logger.warn(
            `[PERF DB] Slow Query: ${modelName}.${operation} took ${Math.round(duration)}ms`,
            {

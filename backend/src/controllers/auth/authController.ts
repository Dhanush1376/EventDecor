...
export const requestUnifiedOtp = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (emailResult.success) {
    ...
    try {
      ...
      const result = await PhoneAuthService.requestOtp(cleanPhone, clientIp);
      challengeId = result.challengeId;
    } catch (_err) {
...
export const verifyOTP = asyncHandler(async (req: Request, res: Response) => {
  ...
  logger.info(`[AUTH] OTP verify requested for challenge ${challengeId}`);
...
export const googleAuth = asyncHandler(async (req: Request, res: Response) => {
  ...
  if ((result as any).action === 'LINK_REQUIRED') {
  ...
  if (r.requires2FA) {
    logger.info(`[AUTH] Google auth verified — awaiting 2FA for user: ${r.user._id}`);
    return res.status(200).json(
      new ApiResponse(true, 'Two-factor authentication required', {
        requires2FA: true,
        userId: r.user._id,

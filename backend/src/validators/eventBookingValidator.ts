...
export const submitEventJobValidator = [
  ...
  body('eventType').trim().notEmpty().withMessage('Event type is required').isLength({ max: 100 }),
  body('date')
    .notEmpty()
  ...
  body('venue.city').optional({ values: 'falsy' }).trim().isLength({ max: 100 }),
  body('venue.state').optional({ values: 'falsy' }).trim().isLength({ max: 100 }),
  body('venue.country').optional({ values: 'falsy' }).trim().isLength({ max: 100 }),
  body('venue.pincode').optional({ values: 'falsy' }).trim().isLength({ max: 20 }),
  body('venue.latitude')
    .optional({ values: 'falsy' })
    .isNumeric()
  ...
  body('venue.googleMapsLink').optional({ values: 'falsy' }).trim().isLength({ max: 1000 }),
  body('venue.isOutdoor').optional().isBoolean().withMessage('isOutdoor must be a boolean value'),
  ...
  body('selectedAddons.*.price').optional().isNumeric().withMessage('Addon price must be a number'),
...
export const customerBookingPaymentValidator = [
  ...
  body('transactionId').optional({ values: 'falsy' }).trim().isLength({ max: 200 }),

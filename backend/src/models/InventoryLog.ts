...
export interface IInventoryLog extends Document {
  ...
  newStock: number;
  delta: number;
  reason: 'order_placed' | 'order_cancelled' | 'payment_failed' | 'stale_release' | 'manual_adjustment' | 'return' | 'restock';
...
const InventoryLogSchema: Schema = new Schema(
  {
    ...
    reason: {
      type: String,
      required: true,
      enum: ['order_placed', 'order_cancelled', 'payment_failed', 'stale_release', 'manual_adjustment', 'return', 'restock'],
    },

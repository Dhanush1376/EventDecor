...
.refine((data) => {
  if (data.NODE_ENV === 'production') {
    ...
    if (data.MONGO_URI.includes('localhost') || data.MONGO_URI.includes('127.0.0.1')) return false;
    
    // Razorpay webhook must be strong
    if (data.RAZORPAY_WEBHOOK_SECRET.length < 32 || /^[a-z_]+$/i.test(data.RAZORPAY_WEBHOOK_SECRET)) return false;
    
    // Redis should be required and use TLS in prod
    if (data.REQUIRE_REDIS === 'true' && (!data.REDIS_URL || !data.REDIS_URL.startsWith('rediss://'))) return false;
  ...
  message: "Production security checks failed. Ensure no 'change_me' secrets, use MongoDB Atlas (no localhost), strong webhook secrets, secure Redis, and disabled dev-only flags."
});

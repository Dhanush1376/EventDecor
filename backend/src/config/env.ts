...
const validateEnv = () => {
  ...
  if (jwtExpiry && !/^\d+[smhd]$/.test(jwtExpiry)) {
    logger.warn(
      `[ENV WARNING] JWT_EXPIRES_IN="${jwtExpiry}" may not be a valid time string. Expected format: 15m, 1h, 7d, etc.`,
    );
  }
  ...
  if (isProduction) {
    ...
    if (process.env.BREVO_API_KEY) {
      ...
      logger.warn(
        '[EMAIL CONFIG] Only SMTP configured — Render free tier may block SMTP ports. Prefer BREVO_API_KEY.',
      );
    }
  }
  ...
  if (missingVars.length > 0) {
    logger.error(
      `[CRITICAL STARTUP ERROR] Missing required environment variables: ${missingVars.join(', ')}`,
    );
    process.exit(1);
  }

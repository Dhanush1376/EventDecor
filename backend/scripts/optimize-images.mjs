...
async function optimizeImages() {
  ...
  if (fs.existsSync(faviconPng)) {
    ...
    await sharp(buffer).resize(128, 128).png({ quality: 80, compressionLevel: 9 }).toFile(faviconPng);
  ...
  if (fs.existsSync(appleIcon)) {
    ...
    await sharp(buffer).resize(180, 180).png({ quality: 80, compressionLevel: 9 }).toFile(appleIcon);
  ...
  if (fs.existsSync(ogImagePng)) {
    ...
    await sharp(buffer).resize(1200, 630, { fit: 'inside' }).png({ quality: 80, compressionLevel: 9 }).toFile(ogImagePng);
  ...
  if (fs.existsSync(ogImageJpg)) {
    ...
    await sharp(buffer).resize(1200, 630, { fit: 'inside' }).jpeg({ quality: 80 }).toFile(ogImageJpg);
